...
contract NumberList {
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `Mappings são tabelas hash que mapeiam chaves para valores. São a forma mais eficiente em termos de gas para armazenar e recuperar dados por chave. Ao contrário de arrays, não é possível iterar sobre mappings nem obter seu comprimento. Todas as chaves possíveis existem com um valor padrão (0 para uint, false para bool, address(0) para address). Mappings aninhados são comuns para relacionamentos complexos.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
contract Phonebook {
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `Este é seu primeiro projeto completo! Você vai construir um contrato de contador e conectá-lo a um frontend React. Isso ensina o padrão fundamental de todos os dApps: implantar um contrato, criar uma ABI e interagir com ela a partir do JavaScript usando ethers.js ou viem. O contador é simples, mas o fluxo de trabalho é o mesmo para aplicações complexas.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
// contract.on("CountChanged", (newCount, changedBy) => { setCount(newCount); });`,
  },
  {
    ...
    explanation: {
      ...
      pt: `Um contrato de armazenamento de mensagens ensina manipulação de strings, emissão de eventos e como frontends exibem dados on-chain. Você vai armazenar mensagens com endereços de remetentes e timestamps, depois lê-las de um frontend React. Este padrão é a base de dApps de redes sociais, sistemas de comentários e fóruns.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
// 5. Listen to MessagePosted events for real-time updates`,
  },
  {
    ...
    explanation: {
      ...
      pt: `O contrato de lista de tarefas combina arrays, structs, mappings e operações CRUD em um projeto prático. Cada usuário tem sua própria lista de tarefas armazenada on-chain. Isso ensina isolamento de dados por usuário, padrões de gerenciamento de estado e como um frontend renderizaria e atualizaria uma lista dinâmica.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
// 5. Listen to events for real-time updates`,
  ...
  // ===== INTERMEDIATE (11-20) =====
  {
    ...
    explanation: {
      ...
      pt: `Structs permitem definir tipos de dados personalizados agrupando variáveis relacionadas. Enums definem um conjunto de constantes nomeadas, perfeito para estados e categorias. Juntos, tornam os contratos mais legíveis e seguros em termos de tipos. Enums são representados internamente como uint8, começando em 0.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
contract TaskManager {
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `Eventos são a ponte entre contratos inteligentes e frontends. Quando emitidos, criam logs armazenados on-chain que são pesquisáveis, mas não acessíveis de dentro dos contratos. Parâmetros indexados (até 3) permitem filtragem eficiente. Eventos custam gas para emitir, mas são muito mais baratos que storage. Frontends usam listeners de eventos para reagir a mudanças no contrato em tempo real.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
contract PaymentProcessor {
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `Modifiers são blocos de código reutilizáveis que envolvem a execução de funções. O placeholder \`_;\` indica onde o corpo da função é executado. São perfeitos para controle de acesso, validação de entrada e verificações de estado. Você pode empilhar múltiplos modifiers em uma única função. Modifiers reduzem duplicação de código e centralizam a lógica de controle de acesso.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
contract SecureContract {
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `Solidity suporta herança simples e múltipla. Funções marcadas como \`virtual\` podem ser sobrescritas por contratos filhos usando \`override\`. Interfaces definem assinaturas de funções sem implementação — contratos devem implementar todas as funções da interface. Contratos abstratos podem ter algumas funções implementadas e outras não. Isso permite um design de contratos modular e reutilizável.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
contract ETHVault is BaseVault {
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `À medida que os projetos crescem, dividir contratos em múltiplos arquivos se torna essencial. Solidity suporta importações nomeadas, importações por wildcard e importações baseadas em caminho. Foundry usa remappings para caminhos de importação mais limpos. Uma boa arquitetura separa interfaces, bibliotecas, contratos base e implementações em arquivos distintos. Isso espelha projetos de nível de produção como o OpenZeppelin.`,
    },
...
contract Vault is Ownable, IVault { function deposit() external payable override {} }`,
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `Controle de acesso vai além de simples verificações de proprietário. O controle de acesso baseado em papéis (RBAC) atribui permissões específicas a endereços. Padrões comuns incluem proprietário único, multi-papel com administrador e acesso em camadas. O AccessControl da OpenZeppelin é o padrão da indústria. Entender esses padrões é crucial para um design de contrato seguro.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
function grantRole(bytes32 r, address a) public onlyRole(ADMIN) { roles[r][a] = true; }`,
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `O framework de testes do Foundry é construído em Solidity. Os testes herdam de \`forge-std/Test.sol\`, que fornece assertions (assertEq, assertTrue), cheatcodes (vm.prank, vm.deal, vm.warp) e logging no console. A função \`setUp()\` é executada antes de cada teste. Entender essas ferramentas é essencial para escrever testes completos.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
function testWithdrawNotOwner() public { vm.prank(bob); vm.expectRevert("Not owner"); vault.withdraw(1 ether); }`,
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `Scripts do Foundry automatizam a implantação e configuração de contratos. Escritos em Solidity, eles usam \`forge-std/Script.sol\` para transmitir transações. Anvil é o nó Ethereum local do Foundry — uma blockchain leve e rápida para desenvolvimento. Juntos, scripts e Anvil possibilitam um fluxo de trabalho de desenvolvimento local completo, equivalente a implantações em produção.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
cast tx <TX_HASH> --rpc-url http://localhost:8545`,
  ...
  },
  {
    ...
    explanation: {
      ...
      pt: `Um sistema de votação combina controle de acesso, structs, enums, eventos e lógica baseada em tempo. Eleitores se registram, propostas são criadas, votos são lançados dentro de uma janela de tempo e resultados são contabilizados. Este projeto ensina padrões de governança usados em DAOs reais. O frontend exibe propostas, permite votação e mostra resultados em tempo real.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
// Frontend: React app with proposal list, vote buttons, result charts.`,
  },
  {
    ...
    explanation: {
      ...
      pt: `Um contrato CRUD (Criar, Ler, Atualizar, Deletar) é um padrão fundamental para gerenciamento de dados on-chain. Este projeto constrói um sistema completo de gerenciamento de registros com paginação, busca por ID, operações em lote e permissões baseadas em proprietário. O frontend fornece uma visualização em tabela com formulários para todas as operações.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
cast call <ADDR> "read(uint256)" 0 --rpc-url http://localhost:8545`,
  ...
  // ===== ADVANCED (21-30) =====
  {
    ...
    explanation: {
      ...
      pt: `O padrão Ownable é o padrão de controle de acesso mais fundamental. Erros customizados (introduzidos no Solidity 0.8.4) são mais eficientes em gas do que strings em require, pois usam codificação por seletor em vez de armazenar dados de string. Também permitem passar parâmetros para melhor depuração. Contratos em produção devem sempre usar erros customizados.`,
    },
    example: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
